...
/*
    ...
    作用：
        当方法内部抛出异常对象时，那马我们就必须处理这个异常对象
        可以使用throws关键字处理异常对象，会把异常对象声明抛出给调用者处理（自己不处理，给别人处理），最终交给JVM处理-->中断处理

    使用格式：在方法声明时使用
        修饰符 返回值类型 方法名（参数列表）throws AAAExpection{
...
public class DemoThrows {
    ...
    /*
        ...
        可以使用throws继续声明抛出FileNotFoundException异常对象，让方法的调用者处理
     */
